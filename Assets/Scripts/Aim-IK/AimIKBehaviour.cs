    ...
    public class AimIKBehaviour : AimIKBehaviourBase
    {
        ...
        private void LateUpdate()
        {
            ...
            if(head.part)
            {
                ...
                {
                    if (SmoothLookAt) // If you checked is smooth option
                        head.part.LookAt3D(SmoothHeadTarget - head.positionOffset, head.rotationOffset);
                    else
                        head.part.LookAt3D(HeadTarget.position - head.positionOffset, head.rotationOffset);
                }
                else if(Target)
                {
                    if (SmoothLookAt) // If you checked is smooth option
